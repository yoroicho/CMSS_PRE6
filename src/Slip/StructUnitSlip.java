...
import DB.UnitDTO;
import UI.FXMLTabPageUnitController;
...
public class StructUnitSlip {
    ...
    public static void creatSlip(UnitDTO unitDTO, FXMLTabPageUnitController fXMLTabPageUnitController) throws IOException, DocumentException, RuntimeException {
        Document document = null; // ZonedDateTime structZonedDateTime = ZonedDateTime.now(ZoneId.systemDefault()); String fileFullDir = UNIT_BASE + FILE_SEPARATOR + String.valueOf(unitDTO.getId()) + FILE_SEPARATOR + String.valueOf(unitDTO.getId()) /*  ???????????????????????????????? UNIT?????pdf?????????????? ???2018-08-15T15:16:10.708+09:00 ????????????? ??Windows????????????????? + "_" 
// + DateTimeFormatter.ISO_OFFSET_DATE_TIME DateTimeFormatter. 
// .format(ZonedDateTime.now(ZoneId.systemDefault())) */ + "-" + DateTimeFormatter.ofPattern("yyyyMMddHHmmssSSS") // .format(structZonedDateTime)  .format(unitDTO.getTimestamp().toLocalDateTime()) + ".pdf"; System.out.println("PDF dir " + fileFullDir); try { // step 1 ???????????????????????0???? document = new Document(PageSize.A4, 90, 90, 90, 90); // step 2  PdfWriter writer = PdfWriter.getInstance( document, new FileOutputStream(fileFullDir) ); // step 3 document.open(); // step 4 PdfContentByte cb = writer.getDirectContent(); 
// ??????????????????? 
document.addTitle(String.valueOf(unitDTO.getId()) +"@" // + DateTimeFormatter.ISO_ZONED_DATE_TIME + DateTimeFormatter.ofPattern("yyyyMMddHHmmssSSS") .format(unitDTO.getTimestamp().toLocalDateTime())); 
// ??????????????????? 
// https://stackoverflow.com/questions/19856583/how-to-add-header-and-footer-to-my-pdf-using-itext-in-java class HeaderFooterPageEvent extends PdfPageEventHelper { @Override public void onStartPage(PdfWriter writer, Document document) { ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase("Top Left"), 30, 800, 0); ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase("Top Right"), 550, 800, 0); } @Override public void onEndPage(PdfWriter writer, Document document) { ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase(String.valueOf(unitDTO.getId()) +"@" + DateTimeFormatter.ofPattern("yyyy'/'MM'/'dd'-'HH':'mm':'ss':'SSS") .format(unitDTO.getTimestamp().toLocalDateTime())), 150, 30, 0); ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase("page " + document.getPageNumber()), 550, 30, 0); } } HeaderFooterPageEvent event = new HeaderFooterPageEvent(); // ?????????????????????????????????? writer.setPageEvent(event); 
// ????????????????? 
// Properties props = new Properties(); String jarPath = System.getProperty("java.class.path"); String dirPath = jarPath.substring(0, jarPath.lastIndexOf(File.separator) + 1); System.out.println(jarPath); System.out.println(dirPath); System.out.println(System.getProperty("user.dir")); Font ipaGothic = new Font(BaseFont.createFont(System.getProperty("user.dir") + FILE_SEPARATOR + "res" + FILE_SEPARATOR + "ipag.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 11); Font ipaGothic14 = new Font(BaseFont.createFont(System.getProperty("user.dir") + FILE_SEPARATOR + "res" + FILE_SEPARATOR + "ipag.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED), 14);  PdfPTable pdfPTable = new PdfPTable(2); pdfPTable.getDefaultCell().setVerticalAlignment(Element.ALIGN_MIDDLE); pdfPTable.getDefaultCell().setHorizontalAlignment(Element.ALIGN_CENTER); pdfPTable.getDefaultCell().setFixedHeight(150); pdfPTable.setWidthPercentage(100f);   int pdfPTableWidth[] // 2???? = {10, 90};  pdfPTable.setWidths(pdfPTableWidth);  PdfPCell cell_1_1 = new PdfPCell(new Paragraph("??", ipaGothic)); cell_1_1.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_1_1.setHorizontalAlignment(Element.ALIGN_CENTER); cell_1_1.setFixedHeight(50); PdfPCell cell_1_2 = new PdfPCell(new Paragraph( overallSeriesId+":"+overallSeriesName, ipaGothic)); cell_1_2.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_1_2.setHorizontalAlignment(Element.ALIGN_CENTER); PdfPCell cell_2_1 = new PdfPCell(new Paragraph("??", ipaGothic)); cell_2_1.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_2_1.setHorizontalAlignment(Element.ALIGN_CENTER); cell_2_1.setFixedHeight(50); PdfPCell cell_2_2 = new PdfPCell(new Paragraph( seriesId+":"+seriesName, ipaGothic)); cell_2_2.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_2_2.setHorizontalAlignment(Element.ALIGN_CENTER);  PdfPCell cell_3_1 = new PdfPCell(new Paragraph("??", ipaGothic)); cell_3_1.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_3_1.setHorizontalAlignment(Element.ALIGN_CENTER); cell_3_1.setFixedHeight(50); PdfPCell cell_3_2 = new PdfPCell(new Paragraph( unitDTO.getMaintitleId()+":"+mainTitleName, ipaGothic)); cell_3_2.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_3_2.setHorizontalAlignment(Element.ALIGN_CENTER);    PdfPCell cell_4_1 = new PdfPCell(new Paragraph("UNIT", ipaGothic)); cell_2_1.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_2_1.setHorizontalAlignment(Element.ALIGN_CENTER); PdfPCell cell_4_2 = new PdfPCell(new Paragraph(unitDTO.toString(), ipaGothic)); cell_2_2.setVerticalAlignment(Element.ALIGN_MIDDLE); cell_2_2.setHorizontalAlignment(Element.ALIGN_CENTER); cell_2_2.setFixedHeight(50);  pdfPTable.addCell(cell_1_1); pdfPTable.addCell(cell_1_2); pdfPTable.addCell(cell_2_1); pdfPTable.addCell(cell_2_2); pdfPTable.addCell(cell_3_1); pdfPTable.addCell(cell_3_2); pdfPTable.addCell(cell_4_1); pdfPTable.addCell(cell_4_2); // pdfPTable.addCell(cell_1_2); /* pdfPTable.addCell(cell_2_1); pdfPTable.addCell(cell_2_2); PdfPCell cellUrlKey = new PdfPCell(new Paragraph("????", ipaGothic)); cellUrlKey.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUrlKey.setHorizontalAlignment(Element.ALIGN_CENTER); cellUrlKey.setRowspan(2); pdfPTable.addCell(cellUrlKey); PdfPCell cellUrlValue = new PdfPCell(new Paragraph(compData, ipaGothic)); Chunk chunk = new Chunk(compData, ipaGothic); // ???????????????????????? System.out.println("????" + chunk.getWidthPoint()); cellUrlValue.setVerticalAlignment(Element.ALIGN_MIDDLE); if (chunk.getWidthPoint() > 410) { 
// ???????????????? cellUrlValue.setHorizontalAlignment(Element.ALIGN_LEFT); } else { 
// ??????????? cellUrlValue.setHorizontalAlignment(Element.ALIGN_CENTER); } cellUrlValue.setFixedHeight(50); pdfPTable.addCell(cellUrlValue); */ Boolean noBarCodePrint = false; // ????????????????? if ((unitDTO.getMaintitleId() + unitDTO.getTitle()).length() != 0 && !noBarCodePrint) { Image image = ZxingUti.getQRCode(unitDTO.toString()); // ????? UTF-8 com.itextpdf.text.Image iTextImage = com.itextpdf.text.Image.getInstance(image, null); PdfPCell cell = new PdfPCell(iTextImage); cell.setVerticalAlignment(Element.ALIGN_MIDDLE); cell.setHorizontalAlignment(Element.ALIGN_CENTER); cell.setFixedHeight(100); pdfPTable.addCell(cell); // ????? UTF-8 } else { PdfPCell cellUrlValueQr = new PdfPCell(new Paragraph("QR??", ipaGothic)); cellUrlValueQr.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUrlValueQr.setHorizontalAlignment(Element.ALIGN_CENTER); cellUrlValueQr.setFixedHeight(80); pdfPTable.addCell(cellUrlValueQr); } /* PdfPCell cellUserNameKey = new PdfPCell(new Paragraph("???", ipaGothic)); cellUserNameKey.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUserNameKey.setHorizontalAlignment(Element.ALIGN_CENTER); cellUserNameKey.setRowspan(2); pdfPTable.addCell(cellUserNameKey); PdfPCell cellUserNameValue = new PdfPCell(new Paragraph(makerName, ipaGothic14)); cellUserNameValue.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUserNameValue.setHorizontalAlignment(Element.ALIGN_CENTER); cellUserNameValue.setFixedHeight(30); pdfPTable.addCell(cellUserNameValue); */ //pdfPTable.addCell("ID");  // EAN??????????????????????????? if (String.valueOf(unitDTO.getId()).length() != 0 && !noBarCodePrint) { Barcode128 code128 = new Barcode128(); code128.setCode(String.valueOf(unitDTO.getId())); code128.setFont(ipaGothic.getBaseFont()); code128.setBarHeight(40f); PdfPCell cellUserNameValueBc = new PdfPCell(code128.createImageWithBarcode(cb, null, null)); cellUserNameValueBc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUserNameValueBc.setHorizontalAlignment(Element.ALIGN_CENTER); cellUserNameValueBc.setFixedHeight(80); pdfPTable.addCell(cellUserNameValueBc); } else { PdfPCell cellUserNameValueBc = new PdfPCell(new Paragraph("---", ipaGothic)); cellUserNameValueBc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellUserNameValueBc.setHorizontalAlignment(Element.ALIGN_CENTER); cellUserNameValueBc.setFixedHeight(80); pdfPTable.addCell(cellUserNameValueBc); } /* PdfPCell cellPassCodeKey = new PdfPCell(new Paragraph("??", ipaGothic)); cellPassCodeKey.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeKey.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeKey.setRowspan(3); pdfPTable.addCell(cellPassCodeKey); PdfPCell cellPassCodeValue = new PdfPCell( new Paragraph(id + "-" + divDateTime, ipaGothic14)); cellPassCodeValue.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeValue.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeValue.setFixedHeight(30); pdfPTable.addCell(cellPassCodeValue); if (id.length() != 0 && !noBarCodePrint) { Barcode128 code128 = new Barcode128(); code128.setCode(id); code128.setFont(ipaGothic.getBaseFont()); code128.setBarHeight(40f); PdfPCell cellPassCodeA_Bc = new PdfPCell(code128.createImageWithBarcode(cb, null, null)); cellPassCodeA_Bc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeA_Bc.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeA_Bc.setFixedHeight(80); pdfPTable.addCell(cellPassCodeA_Bc); } else { PdfPCell cellPassCodeA_Bc = new PdfPCell(new Paragraph("---", ipaGothic)); cellPassCodeA_Bc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeA_Bc.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeA_Bc.setFixedHeight(80); pdfPTable.addCell(cellPassCodeA_Bc); } if (divDateTime.length() != 0 && !noBarCodePrint) { Barcode128 code128 = new Barcode128(); code128.setCode(divDateTime); code128.setFont(ipaGothic.getBaseFont()); code128.setBarHeight(40f); PdfPCell cellPassCodeB_Bc = new PdfPCell(code128.createImageWithBarcode(cb, null, null)); cellPassCodeB_Bc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeB_Bc.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeB_Bc.setFixedHeight(80); pdfPTable.addCell(cellPassCodeB_Bc); } else { PdfPCell cellPassCodeB_Bc = new PdfPCell(new Paragraph("---", ipaGothic)); cellPassCodeB_Bc.setVerticalAlignment(Element.ALIGN_MIDDLE); cellPassCodeB_Bc.setHorizontalAlignment(Element.ALIGN_CENTER); cellPassCodeB_Bc.setFixedHeight(80); pdfPTable.addCell(cellPassCodeB_Bc); } PdfPCell cellCommentKey = new PdfPCell(new Paragraph("??", ipaGothic)); cellCommentKey.setVerticalAlignment(Element.ALIGN_MIDDLE); cellCommentKey.setHorizontalAlignment(Element.ALIGN_CENTER); pdfPTable.addCell(cellCommentKey); PdfPCell cellCommentValue = new PdfPCell(new Paragraph(comment, ipaGothic)); cellCommentValue.setVerticalAlignment(Element.ALIGN_TOP); cellCommentValue.setHorizontalAlignment(Element.ALIGN_LEFT); cellCommentValue.setFixedHeight(150); pdfPTable.addCell(cellCommentValue); PdfPCell cellIssueKey = new PdfPCell(new Paragraph("??", ipaGothic)); cellIssueKey.setVerticalAlignment(Element.ALIGN_MIDDLE); cellIssueKey.setHorizontalAlignment(Element.ALIGN_CENTER); pdfPTable.addCell(cellIssueKey); Calendar cal = Calendar.getInstance(); SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm"); String strDate = sdf.format(cal.getTime()); PdfPCell cellIssueValue = new PdfPCell(new Paragraph(strDate, ipaGothic)); cellIssueValue.setVerticalAlignment(Element.ALIGN_MIDDLE); cellIssueValue.setHorizontalAlignment(Element.ALIGN_CENTER); cellIssueValue.setFixedHeight(20); pdfPTable.addCell(cellIssueValue); */ //????????? document.add(pdfPTable); // step 5 document.close(); System.out.println(fileFullDir + "????????"); } catch (RuntimeException ex) { 
// document.close(); throw ex; } }
